...
class UserAdminViewsTestCase(MVCTestCase):
    ...
    def setUp(self):
        ...
        self.me = self.create_user(username="abcdef", password="abcdef", is_admin=True)
        # login to access admin pages
        data = dict(username="abcdef", password="abcdef")
        self.client.post(url_for("user.login"), data=data, follow_redirects=True)

    def test_user_create_loads(self):
        # Ensure user create admin page loads as expected
        res = self.client.get(url_for("admin.user_create"), follow_redirects=True)
        self.assertEqual(res.status_code, 200)
        # test: create user with complete form data
        data = dict(username="myusername", password="mypassword", email="myusername@test.com")
        res = self.client.post(url_for("admin.user_create"), data=data, follow_redirects=True)
        ...
        data = dict(username="myusername", password="mypassword", email="myusername@test.com")
        res = self.client.post(url_for("admin.user_create"), data=data, follow_redirects=True)
        self.assertIn(b'Username &#34;myusername&#34; already exists.', res.data)
        self.assertIn(b'Email &#34;myusername@test.com&#34; already exists.', res.data)
        # test: create non admin user
        self.assertFalse(user.is_admin)
        # test: create admin user
        data = dict(username="admin", password="adminpassword", email="admin@test.com", is_admin=True)
        self.client.post(url_for("admin.user_create"), data=data, follow_redirects=True)
    ...
    def test_user_detail_view(self):
        ...
        """
        res = self.client.get(url_for("admin.user_detail", user_id=str(self.me)), follow_redirects=True)
        self.assertEqual(res.status_code, 200)
        # test: invalid user id
        res = self.client.get(url_for("admin.user_detail", user_id="_id"), follow_redirects=True)
    ...
    def test_user_toggle_view(self):
        ...
        res = self.client.get(url_for("admin.user_toggle", user_id=str(self.me), action="is_admin"),
        ...
        self.me.save()
        # test: remove user
        res = self.client.get(url_for("admin.user_toggle", user_id=str(self.me), action="remove"),
                              follow_redirects=True)
        self.me.reload()
        ...
        res = self.client.get(url_for("admin.user_toggle", user_id=str("_id"), action="?"),
                              follow_redirects=True)
