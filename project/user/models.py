...
class User(UserMixin, me.Document):
    ...
    def get_user(cls, user):
        ...
        """
        if any([isinstance(user, obj) for obj in [UserMixin, AnonymousUserMixin]]):
            return user
    ...
    @classmethod
    def create(cls, username, password, generate_api_key=True, *args, **kwargs):
        """
        Aux method to create user MongoObject
        """
        kwargs.update(dict(username=username, password=password))
        user = User(**kwargs)
