...
class UserViewsTestCase(MVCTestCase):
    ...
    def test_login_page_loads(self):
        ...
        res = self.client.post(url_for("user.login"), data=data, follow_redirects=True)
    ...
    def test_settings_page_loads(self):
        ...
        res = self.client.get(url_for("user.settings"), follow_redirects=True)
    ...
    def test_register_page(self):
        ...
        res = self.client.get(url_for("user.register"), follow_redirects=True)
        self.assertEqual(res.status_code, 200)
        # test: post request, creates new user with the following data
        data = dict(username="myusername", password=b"mypassword", confirm_password="mypassword")
        res = self.client.post(url_for("user.register"), data=data, follow_redirects=True)
